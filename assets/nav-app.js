...
let projects=[],risingProjects=[],active='__featured',engine='github',lang=localStorage.getItem('lang')||((navigator.language||'').toLowerCase().startsWith('zh')?'zh':'en');
window.__lang=lang;
...
function applyI18n(){document.documentElement.lang=lang==='zh'?'zh-CN':'en'; document.title=lang==='zh'?'拾品号导航 - GitHub 开源项目导航与快速涨星项目榜':'ShipinHao Nav - GitHub Open Source Navigation & Rising Projects'; document.querySelectorAll('[data-i18n]').forEach(el=>el.textContent=t(el.dataset.i18n)); document.querySelectorAll('[data-i18n-html]').forEach(el=>el.innerHTML=t(el.dataset.i18nHtml)); document.querySelectorAll('[data-i18n-placeholder]').forEach(el=>el.placeholder=t(el.dataset.i18nPlaceholder)); const b=$('#langToggle'); if(b)b.textContent=lang==='zh'?'EN':'中文'; const d=$('#dailyLine'); if(d)d.textContent=t('daily'); window.__lang=lang;}
function descText(p){return lang==='zh'?(p.desc_cn||p.desc):p.desc}
function formatStars(n){if(!Number.isFinite(+n))return ''; n=+n; return n>=1000?(n/1000).toFixed(n>=10000?0:1)+'k':String(n)}
function projectMeta(p){const bits=[]; if(p.stars)bits.push(`★ ${formatStars(p.stars)}`); if(p.growth&&p.growth.stars_per_day)bits.push(`↗ ${p.growth.stars_per_day}/天`); if(p.badge)bits.push(p.badge); return bits.length?`<div class="project-meta">${bits.map(x=>`<span>${x}</span>`).join('')}</div>`:''}
function card(p){const desc=descText(p);const tags=(p.tags||[]).slice(0,2).map(x=>`<span>${x}</span>`).join('');return `<a class="link-item" href="${p.url}" target="_blank" rel="noopener" title="${p.name} - ${desc}"><div class="card-head"><span class="site-icon">${p.icon||p.name.slice(0,2).toUpperCase()}</span><div class="title-wrap"><strong>${p.name}</strong><em>${catLabel(p.category)}</em></div></div>${projectMeta(p)}<p class="link-desc">${desc}</p><div class="tag-row">${tags}<b>${t('open')} ↗</b></div></a>`}
function scrollToResults(){const target=$('#browseResults')||$('#activeTitle')||$('#linkGrid'); if(!target)return; target.scrollIntoView({behavior:'smooth',block:'start'}); const title=$('#activeTitle'); if(title){title.classList.remove('jump-highlight'); void title.offsetWidth; title.classList.add('jump-highlight'); setTimeout(()=>title.classList.remove('jump-highlight'),1400);}}
function renderTop(){const nav=$('#topCategories'); if(!nav)return; const list=['__featured',...cats()]; nav.innerHTML=list.map(c=>`<button class="${c===active?'active':''}" data-cat="${c}">${c==='__featured'?t('recommended'):catLabel(c)}</button>`).join(''); nav.querySelectorAll('button').forEach(b=>b.onclick=()=>{active=b.dataset.cat; const inp=$('#searchInput'); if(inp)inp.value=''; renderAll(); requestAnimationFrame(()=>scrollToResults());});}
...
function renderRisingSpotlight(){
  const box=$('#risingSpotlight'); if(!box)return;
  const items=(risingProjects||[]).slice(0,5);
  if(!items.length){box.innerHTML='';return;}
  const cards=items.map((p,i)=>{
    const desc=descText(p);
    const rank=p.growth?.rank||i+1;
    const spd=p.growth?.stars_per_day?Number(p.growth.stars_per_day).toFixed(1):'';
    const tags=(p.tags||[]).filter(x=>x!=='rising').slice(0,2).map(x=>`<span>${x}</span>`).join('');
    return `<a class="rising-card ${i===0?'is-lead':''}" href="${p.url}" target="_blank" rel="noopener"><div class="rising-rank">#${rank}</div><div class="rising-body"><strong>${p.name}</strong><p>${desc}</p><div class="rising-meta"><span>★ ${formatStars(p.stars)}</span>${spd?`<span>↗ ${spd}/天</span>`:''}<span>${catLabel(p.category)}</span></div><div class="rising-tags">${tags}<b>${t('open')} ↗</b></div></div></a>`;
  }).join('');
  box.innerHTML=`<div class="rising-head"><div><span class="eyebrow">GitHub Rising</span><h2>${t('risingTitle')}</h2><p>${t('risingSubtitle')}</p></div><div class="rising-actions"><a href="/daily-brief/">${t('briefShare')} →</a><a href="/trending/">${t('risingMore')} →</a></div></div><div class="rising-grid">${cards}</div>`;
}
function render(){const grid=$('#linkGrid'); if(!grid)return; const {list,q}=filteredList(); grid.innerHTML=list.map(card).join('')||`<p style="grid-column:1/-1;text-align:center;color:#fff">${t('noMatch')}</p>`; $('#activeTitle') && ($('#activeTitle').textContent=currentTitle()); $('#resultCount') && ($('#resultCount').textContent=`${list.length} ${t('count')}`); renderSections(q); renderStats(); renderDailyRecommend(); renderRisingSpotlight();}
...
async function init(){initEffects(); initBackTop(); const [projectData,risingData]=await Promise.all([fetch('/data/projects.json').then(r=>r.json()),fetch('/data/github-rising.json').then(r=>r.ok?r.json():{projects:[]}).catch(()=>({projects:[]}))]); projects=projectData; risingProjects=risingData.projects||[]; if(location.pathname.startsWith('/projects/')) active='__featured'; renderAll(); $('#langToggle')?.addEventListener('click',switchLang); $('#searchInput')?.addEventListener('input',()=>{render();}); $('#searchInput')?.addEventListener('keydown',e=>{if(e.key==='Enter')doSearch();}); $('#searchBtn')?.addEventListener('click',doSearch); $('#friendBtn')?.addEventListener('click',()=>alert(t('friendMsg')));}
init();
